-- adaptive-pipe schema (multi-tenant SaaS MVP)
...
CREATE TABLE runs (
    ...
    file_node_id    TEXT, -- sticky File service instance id (MVP string)
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE stages (
    ...
    name            TEXT NOT NULL, -- validate|file|build|test|deploy
    state           TEXT NOT NULL DEFAULT 'pending', -- pending|skipped|running|succeeded|failed
    started_at      TIMESTAMPTZ,
    finished_at     TIMESTAMPTZ,
    log_ref         TEXT, -- pointer to log segment (MVP optional)
    UNIQUE (run_id, name)
);
